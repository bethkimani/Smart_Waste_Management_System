...
const DriverSignup = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (name && email && password) {
      console.log('Driver Signup attempt:', { name, email });
  ...
  return (
    <section className="flex justify-center items-center h-screen bg-gray-100">
      <div className="max-w-md w-full p-6 rounded-md text-black bg-white">
        <h2 className="text-2xl font-bold mb-4">Driver Signup</h2>
        ...
        <p className="mt-4">
          Already have an account? <Link to="/drivers/login" className="text-green-500">Login</Link>
        </p>
      </div>
    </section>
  );
